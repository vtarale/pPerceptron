...
class Test:
    def __init__(self, learning_rate):
        ...
        self.answers_train = []
        self.answers_test = []
        self.brain = p.Perceptron([0.5, 0.1, 0.1], learning_rate)
    ...
    def split(self, r: int):
        for index in range(r):
            self.train.append([1, self.points[index].x, self.points[index].y])
            self.answers_train.append(self.points[index].lab)
        for index in range(len(self.points) - r):
            self.test.append([1, self.points[index].x, self.points[index].y])
...
if __name__ == "__main__":
    perceptron = Test(10)
    perceptron.create_normal(300, 500000, 5000000)
    perceptron.split(290)
    perceptron.t(5, relu=True)
